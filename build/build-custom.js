...
if (config.features.accessibility) {
  const filePath = path.join(cssDir, 'core', 'accessibility.css');
  if (fs.existsSync(filePath)) {
    const content = fs.readFileSync(filePath, 'utf8');
    imports.push(content);
  } else {
...
const styleguideFilePath = path.join(cssDir, 'presets', 'styleguide.css');
